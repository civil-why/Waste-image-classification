...
testset = datasets.ImageFolder(root='datasets/predict/', transform=data_transform)
    ...
    for epoch in range(epochs):  
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            if i % 50 == 49: # 每50个batch打印一下训练状态
                ...
                for data in predict_l:
                    pictures, labels = data
                    if torch.cuda.is_available():
                         pictures, labels = Variable(pictures.cuda()), Variable(labels.cuda())
                    else:
                         pictures, labels = Variable(pictures), Variable(labels)
                    outputs = net(pictures)
                    _, predicted = torch.max(outputs, 1)
                    total += labels.size(0)
                    correct += (predicted == labels).sum()                
                
                for train_data in trainloader:
                    pictures, labels = train_data
                    if torch.cuda.is_available():
                      pictures, labels = Variable(pictures.cuda()), Variable(labels.cuda())
                    else:
                      pictures, labels = Variable(pictures), Variable(labels)
...
def predict():
    ...
    nerve_net = torch.load(model)#加载神经网络模型
    nerve_net.eval()
    
    with torch.no_grad():#关闭自动求导以节约显存
        for data in predict_l:
            pictures, labels = data
            if torch.cuda.is_available():
                pictures, labels = Variable(pictures.cuda()), Variable(labels.cuda())
                ...
                pictures, labels = Variable(pictures), Variable(labels)
            outputs = nerve_net(pictures)
            _, predicted = torch.max(outputs, 1)
    
    print('预测结果：')
    for i in range(len(predicted)):
        print('第{0}张图片为：{1}'.format(i,classes[predicted[i]]))
